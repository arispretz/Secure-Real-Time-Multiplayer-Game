...
module.exports = function (socket, io) {
  ...
  io.on("connection", (socket) => {
    ...
    socket.emit(
      "new game",
      { x: player.x, y: player.y, score: player.score, id: player.id },
      collectible,
      Array.from(usersMap, ([name, score]) => ({ name, score }))
    );
...
function setCollectWOCollision(player, coord) {
  if (coord == "x") {
    ...
    while (
      (x + collectibleWidth > player.x &&
        x + collectibleWidth < player.x + playerWidth) ||
    ...
    while (
      (y + collectibleHeight > player.y &&
        y + collectibleHeight < player.y + playerHeight) ||
