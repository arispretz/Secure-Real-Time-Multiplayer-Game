...
const gamerColor = "green";
const coinColor = "purple";
...
class Player {
  ...
  collision(item) {
    if (
      item.x >= this.x &&
      item.x <= this.x + playerWidth &&
      item.y >= this.y &&
      item.y <= this.y + playerHeight
    ) {
      return true;
    } else if (
      item.x + collectibleWidth >= this.x &&
      item.x + collectibleWidth <= this.x + playerWidth &&
      item.y >= this.y &&
      item.y <= this.y + playerHeight
    ) {
      return true;
      ...
      item.x >= this.x &&
      item.x <= this.x + playerWidth &&
      item.y + collectibleHeight >= this.y &&
      item.y + collectibleHeight <= this.y + playerHeight
    ) {
      return true;
    } else if (
      item.x + collectibleWidth >= this.x &&
  ...
  draw(canvas, context, coin, img, usersArray, coinImg) {
    ...
    document.getElementsByTagName(
      "h1"
    )[0].innerHTML = `Secure Real Time Multiplayer   Game ${this.calculateRank(
      usersArray
    )}   Score ${this.score}`;
